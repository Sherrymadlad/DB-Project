...
const RestaurantDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [restaurantRes, ratingRes, likedRes, imageRes] =
          await Promise.all([
        ...
        <div className="w-60 aspect-square rounded-full overflow-hidden border-4 border-theme-pink shadow-md flex items-center justify-center">
          <img
            ...
            className={`object-cover transition-all duration-300 ${
              restaurant.profilePicUrl ? "w-full h-full" : "w-[90%] h-[90%]"
            }`}
          />
        </div>
        ...
        <div className="flex justify-between items-start w-full">
          <div className="flex flex-col gap-2 w-full max-w-2xl relative">
            ...
            <p>
              ...
              {cuisines.length > 0
                ? cuisines.map((c) => c.CuisineName).join(", ")
                : "-"}
            </p>
            <p>
              <strong>Operating Hours:</strong>{" "}
              {formatTime(restaurant.OperatingHoursStart)} -{" "}
              {formatTime(restaurant.OperatingHoursEnd)}
            </p>
            <p>
          ...
          <div className="flex flex-col items-end gap-2 mr-20">
            ...
            <div className="text-theme-pink font-semibold text-4xl mt-4">
              {rating.toFixed(1)}
            </div>
