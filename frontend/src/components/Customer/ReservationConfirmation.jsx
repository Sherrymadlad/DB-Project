  ...
  const fetchTables = async (capacityValue) => {
    try {
      const res = await axios.get(
        ...
        { params: { minCapacity: capacityValue && !isNaN(capacityValue) ? parseInt(capacityValue) : 0, startTime: dateTime, duartionMinutes: duration } }
      );
      setTables(res.data);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 relative">
      ...
      <div className="flex flex-wrap gap-4 justify-center mb-6">
        <div>
          <label className="block text-center text-sm font-semibold text-gray-700 mb-1">Minimum Capacity</label>
          <input
            type="number"
            min="0"
      ...
      {tables.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tables.map((table, key) => (
            <div key={table.id} className="p-4 rounded shadow-md border border-gray-200">
              <h2 className="text-lg font-semibold text-gray-800 mb-2">Table {key + 1}</h2>
              <p className="text-sm text-gray-600 mb-1"><strong>Capacity:</strong> {table.Capacity} seats</p>
              <p className="text-sm text-gray-600 mb-3"><strong>Description:</strong> {table.Description}</p>
        ...
        <p className="text-center text-gray-500 text-lg mt-10">No tables available.</p>
      )}
    </div>
  );
};
